    ...
    while post < max:
        if not os.path.exists(search):
            if dirName:
                ...
                os.mkdir(search)
        url = "https://www.fmkorea.com/search.php?mid=humor&search_keyword=" + search + "&search_target=title_content&page=" + str(
            page)
                ...
                for video in video_tags:
                    ...
                    if source_tag:
                        ...
                        if src is not None:
                            ...
                            if dirName == None:
                                filepath = os.path.join(search, filename)
                            else:
                                filepath = os.path.join(dirName, search, filename)
                            try:
...
def validate_and_run():
    ...
    file_count_text = fnEntry.get()

    if not searchtext:
        msgbox.showerror("Error", "검색어를 입력하세요.")
        return
    if not file_count_text:
        msgbox.showerror("Error", "파일 개수를 입력하세요.")
        return
...
if __name__ == "__main__":
    ...
    fnLabel = Label(window, text="게시글 개수", font=fonts)  # 게시글 개수
    fnLabel.place(x=100, y=40)

    searchEntry = Entry(width=30)  # 검색어 입력창
    searchEntry.place(x=250, y=10)
    searchEntry.bind('<Key>', on_key)

    fnEntry = Entry(width=30)  # 게시글 개수 입력창
    fnEntry.place(x=250, y=40)
